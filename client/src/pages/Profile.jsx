    ...
    <div className='p-3 max-w-lg mx-auto'>
      ...
      <form onSubmit={handleSubmit} className='flex flex-col gap-4'>
        ...
        <input
          ...
          placeholder='email'
          id='email'
          defaultValue={updateCurrentUser.email}
          className='border p-3 rounded-lg'
          onChange={handleChange}
        />
        <input
      ...
      {userListings.length > 0 && <div className=" flex flex-col gap-4">
        <h1 className='text-center mt-7 text-2xl font-semibold'>Your Listings</h1> {userListings.map((listing) => (
          <div key={listing._id} className="border rounded-lg p-3 flex justify-between items-center gap-4">
            <Link to={`/listing/${listing._id}`}>
              <img src={listing.imageUrls[0]} alt="listing cover" className='h-16 w-16 object-contain' />
              <h2 className='text-xl font-semibold'>{listing.title}</h2>
            </Link>
            <Link className='flex-1 text-slate-700 font-semibold hover:underline truncate' to={`/listing/${listing._id}`}> <p></p>{listing.name}
            </Link>
